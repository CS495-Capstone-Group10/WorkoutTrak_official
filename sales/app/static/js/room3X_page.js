...
function MyComponent() {
  ...
  const [firstName, setFirstName] = React.useState(localStorage.getItem('firstName3') || '');
  const [lastName, setLastName] = React.useState(localStorage.getItem('lastName3') || '');
  const [gender, setGender] = React.useState(localStorage.getItem('gender3') || '');
  const [email, setEmail] = React.useState(localStorage.getItem('email') || '');
  const [occupation, setOccupation] = React.useState(localStorage.getItem('occupation') || '');

  const handleFirstNameChange = (event) => {
    setFirstName(event.target.value);
    localStorage.setItem('firstName3', event.target.value);
  };

  const handleLastNameChange = (event) => {
    setLastName(event.target.value);
    localStorage.setItem('lastName3', event.target.value);
  };

  const handleGenderChange = (event) => {
    setGender(event.target.value);
    localStorage.setItem('gender3', event.target.value);
  };
        ...
        <nav className="navbar">
          <div className="navbar__container">
            <a href="/homeX" id="navbar__logo">
              <i className="fas fa-gem"></i>WorkoutTrak : Room 3
            </a>
